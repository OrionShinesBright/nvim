...
function M.compile_and_run()
  ...
  if vim.v.shell_error == 0 then
    local run = string.format('silent !dosbox -noconsole -conf ~/.config/dosbox/dosbox-vimasm.conf -c "mount d %s" -c "d:" -c "%s"', dir, vim.fn.fnamemodify(com,":t"))
    vim.cmd(run)
  else
...
function M.compile_and_debug()
  ...
  vim.cmd("silent !/usr/bin/nasm -f bin " .. vim.fn.shellescape(asm) .. " -o " .. vim.fn.shellescape(com) .. " -l " .. vim.fn.shellescape(lst))
  if vim.v.shell_error == 0 then
    local run = string.format('silent !dosbox -noconsole -conf ~/.config/dosbox/dosbox-vimasm.conf -c "mount d %s" -c "d:" -c "c:\\dos\\afd %s"', dir, vim.fn.fnamemodify(com,":t"))
    vim.cmd(run)
  else
    vim.cmd("echo 'Compilation failed'")
  end
